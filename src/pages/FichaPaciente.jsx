...
const FichaPaciente = () => {
  ...
  const consultasConcluidas = agendamentosPaciente.filter((item) =>
    ['CONCLUIDO', 'FINALIZADO', 'CONCLUIDA'].includes(item.statusConsulta?.toUpperCase())
  ).length;
  ...
  if (erroPaciente) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[24rem]">
        ...
        <h2 className="text-xl font-bold text-dentista-title mb-2">Erro ao carregar paciente</h2>
  ...
  if (!pacienteSelecionado) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[24rem]">
        ...
        <h2 className="text-xl font-bold text-dentista-title mb-2">Paciente não encontrado</h2>
        <BackButton />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto pb-10 px-4">
      ...
      <ProfileHeader
        ...
        fields={[
          { label: 'Idade', value: idadePaciente !== null ? `${idadePaciente} anos` : 'Não informado' },
        ...
        <div className="flex flex-wrap gap-2">
          {TAB_ITEMS.map((tab) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition ${activeTab === tab.id ? 'bg-dentista-primary text-white' : 'bg-gray-100 text-dentista-body hover:bg-gray-200'}`}
            >
              {tab.label}
            </button>
          ))}
      ...
      <div className="bg-white rounded-clinica shadow-sm border border-gray-100 p-6">
        {activeTab === 'ultimas' && (
          <>
            <div className="mb-4">
              <h3 className="text-xl font-semibold text-dentista-title">Últimas Consultas</h3>
              <p className="text-sm text-dentista-body">Registros de consultas vinculadas a este paciente.</p>
            </div>
            {agendamentosPaciente.length > 0 ? (
                  ...
                  <tbody>
                    {agendamentosPaciente.map((consulta) => (
                      <tr key={consulta.id} className="bg-gray-50 rounded-3xl">
                        <td className="px-4 py-4 align-top">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-dentista-primary">
                              {consulta.nomePaciente?.charAt(0).toUpperCase() || pacienteSelecionado.nome?.charAt(0).toUpperCase() || 'P'}
                            </div>
                            <div>
                              <p className="font-semibold text-dentista-title">{consulta.nomePaciente || pacienteSelecionado.nome}</p>
                              <p className="text-sm text-gray-500">{consulta.pacienteTelefone || pacienteSelecionado.telefone || 'Telefone não informado'}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 align-top text-sm">
                          <span className={`inline-flex rounded-full px-3 py-1 font-semibold ${getStatusBadge(consulta.statusConsulta)}`}>
              ...
              <div className="py-10 text-center text-dentista-body">Nenhuma consulta encontrada para este paciente.</div>
            )}
          </>
        )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <SectionCard title="Informações Básicas">
              <div className="space-y-3 text-sm text-dentista-body">
                <p><span className="font-semibold">CPF:</span> {pacienteSelecionado.cpf || 'Não informado'}</p>
                <p><span className="font-semibold">Data de nascimento:</span> {pacienteSelecionado.dataNascimento || 'Não informado'}</p>
                <p><span className="font-semibold">Sexo:</span> {pacienteSelecionado.sexo || 'Não informado'}</p>
                <p><span className="font-semibold">Cadastrado em:</span> {pacienteSelecionado.criadoEm || 'Não informado'}</p>
              </div>
            </SectionCard>
            <SectionCard title="Contato">
              <div className="space-y-3 text-sm text-dentista-body">
                <p className="flex items-center gap-2"><Phone size={16} className="text-dentista-primary" /> {pacienteSelecionado.telefone || 'Não informado'}</p>
                <p className="flex items-center gap-2"><Mail size={16} className="text-dentista-primary" /> {pacienteSelecionado.email || 'Não informado'}</p>
              </div>
            </SectionCard>
            <SectionCard title="Endereço">
              <p className="text-sm text-dentista-body">{pacienteSelecionado.endereco || 'Endereço não informado'}</p>
            </SectionCard>
          </div>
        )}
      </div>
